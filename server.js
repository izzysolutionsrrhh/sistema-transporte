...
app.post('/api/solicitudes', solicitudLimiter, async (req, res) => {
  const { nombre_empresa, ruc_o_cedula, direccion, contacto_nombre, contacto_email, contacto_telefono } = req.body;
  if (!nombre_empresa?.trim() || !ruc_o_cedula?.trim() || !direccion?.trim() || !contacto_nombre?.trim())
